		...
		try
		{
			...
			for(Mosquito m : board.getMosquitos())
			{
				if(!m.caught)
				{
					...
					Collector c = board.getCollector();
					if(c.contains(m))
					{
						m.caught = true;
...
//			System.out.println("ID " + tournament_id);
	...
	public boolean setUpGame()
	{
		try
		{
			...
			board.createMosquitos(0); // TODO: do we need this?
			...
			Collector collector = curPlayer.getCollector(); 
			if(collector == null)
			{
				System.err.println("Error: Player returned null for collector");
				return false;
			}
			for(Line2D w : board.getWalls())
			{
				if (collector.intersects(w)) {
			...
			for(Light l : lights)
			{
				if(collector.contains(l.getLocation()))
				{
					System.err.println("Error: Collector intersects light");
					return false;
				}
			}
			if(collector.getX() < 0 || collector.getX() > 100 || collector.getY() < 0 || collector.getY() > 100)
			{
				System.err.println("Error: Collector is OOB");
				return false;
			}
			
			board.setCollector(collector); 

			if(!this.isSimulated)
			{ // TODO: do we still need this?
				GameEngine.lights = lights;
				GameEngine.collector = collector;
			}
			initDone = true;
